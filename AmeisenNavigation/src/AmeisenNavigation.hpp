...
public:
    ...
    AmeisenNavigation(const std::string& meshFolder, int maxPolyPath, int maxSearchNodes, bool useAnp = false,
        ...
        : MaxPolyPath(maxPolyPath), MaxSearchNodes(maxSearchNodes), Clients()
    {
        if (useAnp)
        {
            NavSource = new AnpNavSource(meshFolder.c_str());
            FilterProvider = new AnpQueryFilterProvider(1.6f, 4.0f, 0.75f, factionDangerCost);
        }
        else
        {
    ...
    bool GetRandomPath(size_t clientId, int mapId, const Vector3& startPosition, const Vector3& endPosition, Path& path,
                       float maxRandomDistance) noexcept;
    ...
    bool MoveAlongSurface(size_t clientId, int mapId, const Vector3& startPosition, const Vector3& endPosition,
                          Vector3& positionToGoTo) noexcept;
    ...
    bool CastMovementRay(size_t clientId, int mapId, const Vector3& startPosition, const Vector3& endPosition,
                         dtRaycastHit* raycastHit) noexcept;

    /// <summary>
    /// Create a path to explore a polygon. Path will cover the area of the polygon and be created using a TSP algorithm
    ...
    /// <returns>True when a path was found, false if not.</returns>
    bool GetPolyExplorationPath(size_t clientId, int mapId, const Vector3* polyPoints, int inputSize, Path& path,
                                Path& tmpPath, const Vector3& startPosition, float viewDistance) noexcept;
...
private:
    ...
    /// </summary>
    inline bool IsValidClient(size_t clientId) noexcept { std::shared_lock lock(ClientsMutex); auto it = Clients.find(clientId); return it != Clients.end() && it->second != nullptr; }
